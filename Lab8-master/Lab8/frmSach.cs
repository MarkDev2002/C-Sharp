...
namespace Lab8
{
    public partial class frmSach : Form
    {
        ...
        public bool CheckData()
        {
            ...
            {
                MessageBox.Show("Trị giá chỉ nhập giá trị số!");
                txtTriGia.Focus();
                return false;
            }
            return true;
        ...
        private void BtnLuu_Click(object sender, EventArgs e)
        {
            if (CheckData())
            {
                if (themmoi)
                {
                    s.ThemSach(txtTenSach.Text, txtTacGia.Text, int.Parse(txtNamXB.Text), txtNhaXB.Text, float.Parse(txtTriGia.Text), dateTimePicker.Value);
                    MessageBox.Show("Thêm mới thành công!");
                }
                else
                {
                    s.CapNhatSach(dgvSach.SelectedRows[0].Cells[0].Value.ToString(), txtTenSach.Text, txtTacGia.Text, int.Parse(txtNamXB.Text), txtNhaXB.Text, float.Parse(txtTriGia.Text), dateTimePicker.Value);
