...
namespace DAO
{
    public class CustomerDAO : DataConection
    {
        ...
        public List<CustomerDTO> ShowListCus()
        {
            ...
            cmd.CommandText = "Select *from CUSTOMER where Status = 'ACT'";
            cmd.Connection = conn;
		...
		public bool AddGuest(string id)
		{
			Moketnoi();
			MySqlCommand cmd = new MySqlCommand();
			cmd.CommandType = CommandType.Text;
			cmd.CommandText = "insert into CUSTOMER(CUSTOMERID,CUSTOMERSURNAME,CUSTOMERNAME,GENDER,PHONE,ADDRESS,Status) Values('"+id+"','GUEST','GUEST','GUEST','GUEST','GUEST','GST')";
			cmd.Connection = conn;
		...
		public bool DeleteCus(string id)
        {
            ...
            MySqlCommand cmd = new MySqlCommand();
            cmd.CommandType = CommandType.Text;
            cmd.CommandText = "update CUSTOMER set Status = 'DEL' where CUSTOMERID =@CusID";
            cmd.Connection = conn;
        ...
        public bool AppCus(CustomerDTO cus)
        {
            ...
            MySqlCommand cmd = new MySqlCommand();
            cmd.CommandType = CommandType.Text;
            cmd.CommandText = "insert into CUSTOMER(CUSTOMERID,CUSTOMERSURNAME,CUSTOMERNAME,GENDER,PHONE,ADDRESS,Status) Values(@id,@surname,@name,@gender,@phone,@address,'ACT')";
            cmd.Connection = conn;
        ...
        public List<CustomerDTO> SeachCus(string strSeach)
        {
            ...
            MySqlCommand cmd = new MySqlCommand();
            cmd.CommandType = CommandType.Text;
            cmd.CommandText = "select *from CUSTOMER where CONCAT(CUSTOMERID,CUSTOMERSURNAME,CUSTOMERNAME,GENDER,PHONE,ADDRESS) like '%" + strSeach + "%' and Status = 'ACT'";
            cmd.Connection = conn;
            MySqlDataReader reader = cmd.ExecuteReader();
            while (reader.Read())
            {
