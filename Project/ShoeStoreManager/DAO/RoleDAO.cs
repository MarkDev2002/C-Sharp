...
namespace DAO
{
    public class RoleDAO
    {
		...
		public DataTable GetDataWithoutDefault()
		{
			DataTable dt = new DataTable();

			string query = "SELECT * FROM role where not roleID = 'ROL000' and not roleid = 'ROL001'";
        ...
        {
			string query_0 = string.Format("Update Account set roleid = 'ROL000' WHERE RoleID = '{0}'", roldID);
			DataProvider.Instance.ExecuteNonQuery(query_0);
