...
namespace DAO
{
    public class SupplierDAO : DataConection
    {
        ...
        public List<SupplierDTO> ShowListSuppier()
        {
            ...
            cmd.CommandText = "Select *from SUPPLIER where Status = 'ACT'";
            cmd.Connection = conn;
        ...
        public bool DeleteSup(string id)
        {
            ...
            MySqlCommand cmd = new MySqlCommand();
            cmd.CommandType = CommandType.Text;
            cmd.CommandText = "update SUPPLIER set status = 'DEL' where SUPPLIERID =@SupID";
            cmd.Connection = conn;
        ...
        public bool AppSup(SupplierDTO sup)
        {
            ...
            MySqlCommand cmd = new MySqlCommand();
            cmd.CommandType = CommandType.Text;
            cmd.CommandText = "insert into SUPPLIER(SUPPLIERID,SUPPLIERNAME,ADDRESS,PHONE,FAXNUMBER, status) Values(@id,@supname,@address,@phone,@faxnumber,'ACT')";
            cmd.Connection = conn;
        ...
        public List<SupplierDTO> SeachSup(string strSeach)
        {
            ...
            MySqlCommand cmd = new MySqlCommand();
            cmd.CommandType = CommandType.Text;
            cmd.CommandText = "select *from SUPPLIER where CONCAT(SUPPIERID,SUPPIERNAME,ADDRESS,PHONE,FAXNUMBER) like '%" + strSeach + "%' and status = 'ACT'"; ;
            cmd.Connection = conn;
            MySqlDataReader reader = cmd.ExecuteReader();
            while (reader.Read())
            {
