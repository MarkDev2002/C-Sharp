...
namespace GUI
{
    public partial class AccountGUI : Form
    {
        ...
        public void LoadListAccount()
        {
            ...
            dtgvAccount.Columns["colDelete"].DisplayIndex = 0;
			dtgvAccount.Columns["colUpdate"].DisplayIndex = 0;
			dtgvAccount.Columns["Enable"].Visible = false;
			ChangeColHeader();
            ChangeColWidth();
        }
		...
		private bool ValidateData()
		{
			...
			{
				messError.Show("Nhập password");
				return false;
			}

			if (string.IsNullOrEmpty(txtEmail.Text))
			{
				messError.Show("Nhập email");
				return false;
			}
		...
		private void btnOK_Click(object sender, EventArgs e)
        {
            try
            {
				...
				if (InsertOrUpdate)
                {
                    ...
                    if (result == DialogResult.Yes)
                    {
                        if (AccountDAO.Instance.InsertAccount(txtUserID.Text.ToString(), txtUsername.Text.ToString(), txtPassword.Text.ToString(), cmbRole.SelectedValue.ToString(), txtEmail.Text.ToString(), cmbEnable.SelectedItem.ToString()) != false)
                        {
                            DefaultDisplay();
		...
		private void cmbEnable_SelectedIndexChanged(object sender, EventArgs e)
		{
