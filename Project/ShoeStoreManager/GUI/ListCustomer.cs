...
namespace GUI
{
    public partial class ListCustomer : KryptonForm
    {
        ...
        public void DisableColumn()
        {
            ...
            dtgv.Columns["Status1"].Visible = false;
        }
        ...
        {
            _sale.lbID.Text = dtgv.Rows[e.RowIndex].Cells["CustomerSurname1"].Value.ToString()
                + " " + dtgv.Rows[e.RowIndex].Cells["CustomerName1"].Value.ToString();
            _sale.lbName.Text = dtgv.Rows[e.RowIndex].Cells["Address1"].Value.ToString();
            _sale.lbPhone.Text = dtgv.Rows[e.RowIndex].Cells["PhoneNumber1"].Value.ToString();

            _sale.customerID = dtgv.Rows[e.RowIndex].Cells["CustomerID1"].Value.ToString();
            _sale.label1.Text = "CUSTOMER: " + dtgv.Rows[e.RowIndex].Cells["CustomerSurname1"].Value.ToString().ToUpper()
				+ " " + dtgv.Rows[e.RowIndex].Cells["CustomerName1"].Value.ToString().ToUpper();
            this.Hide();
        }
    }
}
