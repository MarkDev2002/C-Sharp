...
namespace GUI
{
    public partial class SupplierGUI : Form
    {
        ...
        {
            InitializeComponent();
            txtID_item.Text = SupplierBUS.Instance.SetAutoID();
		...
		private void BtnAdd_item_Click(object sender, EventArgs e)
        {
            ...
            if (kq == "error_phone")
            {
                MessageBox.Show("Sai định dạng số điện thoại (Số điện thoại gồm 10 chữ số (từ 0 - 9))", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            }
            if (kq == "error_fax")
            {
        ...
        private void BtnUpd_Click(object sender, EventArgs e)
        {
			...
			//    MessageBox.Show("Sai định dạng mã", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Warning);
			//}
			if (kq == "error_SupId_isNull" || kq == "error_Supname" || kq == "error_Phonenull" || kq == "error_Address")
            {
                MessageBox.Show("Vui lòng nhập đầy đủ thông tin", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            }
            if (kq == "error_phone")
        ...
        {
            CreateImport f = new CreateImport();
            this.Hide();
            f.ShowDialog();
        }
    }
